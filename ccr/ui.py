...
def inject_css():
    ...
    /* Keep main content from sliding under the fixed panel */
    @media (min-width: 1100px) {
      #left-params { max-width: 900px; margin-right: 440px; }
    }

    /* Hide the Streamlit sidebar and its toggle/collapse control */
...
def stepper():
    try:
        st.session_state.step = st.segmented_control(
            "Step",
            options=["Campaign Information","Evaluation","Results"],
            default=st.session_state.step
        )
    except Exception:
        st.session_state.step = st.radio(
            "Step",
            ["Campaign Information","Evaluation","Results"],
            index=["Campaign Information","Evaluation","Results"].index(st.session_state.step),
            horizontal=True
        )
...
def page_campaign_info():
    ...
    with c_left:
        ...
        st.session_state.info["channel"] = st.selectbox(
    ...
    with c_right:
        st.session_state.info["scene_audience_choice"] = st.selectbox(
        ...
        st.session_state.info["rater_id"] = st.selectbox(
            "Rater", ["Lode","Maarten"],
            index=["Lode","Maarten"].index(st.session_state.info["rater_id"]) if st.session_state.info["rater_id"] in ["Lode","Maarten"] else 0
        )
    st.session_state.info["asset_youtube_url"] = st.text_input(
        "YouTube URL (optional)",
        value=st.session_state.info["asset_youtube_url"],
        placeholder="https://www.youtube.com/watch?v=..."
    )
    st.session_state.info["rater_notes"] = st.text_area(
        "Rater notes",
        value=st.session_state.info["rater_notes"],
        height=90,
        placeholder="Key observations, insider cues, etc."
    )
...
def sliders_and_risks():
    ...
    st.session_state.risks["neg_sentiment_ratio_estimate"] = st.slider(
        "Negative sentiment ratio (0–1)", 0.0, 1.0, float(st.session_state.risks["neg_sentiment_ratio_estimate"]), 0.01
    )
